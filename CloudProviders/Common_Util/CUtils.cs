...
public static class CUtils
{
    public static T CreateDefault<T>()
    {
        ...
        return CUtils.CreateEmptyArray<T>();
    }
    ...
    public static T CreateEmptyArray<T>()
    {
        ...
        return (T)Array.CreateInstance(type.GetElementType(), numArray);
    }
    ...
    public static string PathToUNC(string serverName, string inPath)
    {
        ...
        try
        {
            ...
            {
                str1 = IPHelper.GetWNetServerString(serverName.TrimStart('\\'));
                if (!string.IsNullOrEmpty(inPath))
                {
                    ...
                    if (Path.IsPathRooted(inPath))
                    {
                        ...
                        string str2 = inPath.Substring(pathRoot.Length);
                        str1 = str1 + "\\" + pathRoot.Substring(0, 1) + "$\\" + str2;
                    }
                }
            }
        }
    ...
    public static string GetSharePathFromUnc(string fullPath)
    {
        ...
        CUtils.GetShareInfoFromUnc(fullPath, out serverName, out shareName);
    ...
    public static bool PingServer(string serverName, int numberOfAttempts)
    {
        ...
        {
            if (CUtils.PingServer(serverName))
            {
                flag = true;
                break;
            }
    ...
    public static void WaitForReboot(string serverName, int interval, int timeout)
    {
        ...
        return;
        label_6:
        throw new OculiServiceException(0, "Timeout occurred while waiting for server " + serverName + " to reboot");
    }
    ...
    {
        return CUtils.RunShellCmd(cmd, args, int.MaxValue, out stdout, out stderr);
    }
    ...
    public static int RunShellCmd(string cmd, string args, int maxwait, out string stdout, out string stderr)
    {
        if (!((IEnumerable<string>)new string[3] { "zh", "ja", "ko" }).Contains<string>(CultureInfo.CurrentCulture.TwoLetterISOLanguageName))
            return CUtils.RunShellCmdStdOut(cmd, args, maxwait, out stdout, out stderr);
        int num = CUtils.RunShellCmdTimeout(cmd, args, maxwait);
        stdout = "";
        stderr = "";
        return num;
    }
    ...
    public static int RunShellCmdLog(string cmd, string args, int maxwait, ILogger logger)
    {
        logger.Verbose(string.Format("Executing:  {0} {1}", (object)cmd, (object)args));
        using (Process process1 = new Process())
        {
            process1.StartInfo.UseShellExecute = false;
            process1.StartInfo.FileName = cmd;
            ...
            DataReceivedEventHandler receivedEventHandler1 = (DataReceivedEventHandler)((sendingProcess, outLine) =>
           {
               if (string.IsNullOrEmpty(outLine.Data))
                   return;
               logger.Verbose(string.Format("StdOut({0}): {1}", (object)cmd, (object)outLine.Data));
           });
            process1.OutputDataReceived += receivedEventHandler1;
            process1.StartInfo.RedirectStandardError = true;
            DataReceivedEventHandler receivedEventHandler2 = (DataReceivedEventHandler)((sendingProcess, outLine) =>
           {
               ...
               logger.Verbose(string.Format("StdErr({0}): {1}", (object)cmd, (object)outLine.Data));
           });
            process1.ErrorDataReceived += receivedEventHandler2;
            try
            {
    ...
    {
        return CUtils.RunShellCmdTimeout(cmd, args, int.MaxValue);
    }
    ...
    public static Uri TrimDomainFromFQDN(Uri serverUri)
    {
        return new HostUriBuilder(serverUri) { NetworkId = CUtils.TrimDomainFromFQDN(serverUri.Host) }.Uri;
    }
    ...
    public static bool IsSameFolder(string path1, string path2)
    {
        ...
        string path2_1 = CUtils.NewGuid().ToString();
        bool flag = false;
    ...
    public static void Retry(int attempts, int Wait, CUtils.Workload work, CUtils.Workload reset)
    {
    ...
    public static ProductVersion GetHyperVVersion()
    {
        ...
        if (System.IO.File.Exists(str))
        {
            ...
            productVersion.Major = versionInfo.ProductMajorPart;
            productVersion.Minor = versionInfo.ProductMinorPart;
            productVersion.Build = versionInfo.ProductBuildPart;
        }
    ...
    public static string GetMD5(string fileName)
    {
        ...
        try
        {
            ...
            return CUtils.BytesToStr(cryptoServiceProvider.Hash);
        }
        finally
        {
            if (fileStream != null)
    ...
    public static string GetVimUuidFromBiosUuid(Guid uuid)
    {
        if (uuid == Guid.Empty)
            return (string)null;
        return CUtils.GetVimUuidFromBiosUuid(uuid.ToString());
    }

    public static bool DiskPart(string script, object diskPartLock, int timeoutMilliseconds)
    {
        return CUtils.DiskPart(script, diskPartLock, (ILogger)new Logger_Trace(), timeoutMilliseconds);
    }

    public static bool DiskPart(string script, object diskPartLock, ILogger logger, int timeoutMilliseconds)
    {
        lock (diskPartLock)
        {
            ...
            logger.Verbose("========== Contents of script ==========");
            using (StreamReader resource_1 = new StreamReader(local_2))
            {
                while (resource_1.Peek() >= 0)
                    logger.Verbose(resource_1.ReadLine());
            }
            logger.Verbose("========== End of script ==========");
            ...
            try
            {
                ...
                {
                    if (local_9 == 0)
                        logger.Verbose("Running diskpart");
                    else
                        logger.Verbose("Retrying diskpart");
                    local_8 = CUtils.RunShellCmdLog("diskpart", local_3, timeoutMilliseconds, logger);
                    logger.Verbose(string.Format("diskpart returned exit code {0:d}", (object)local_8));
                    logger.Verbose(local_4);
                    logger.Verbose(local_5);
                    Thread.Sleep(15000);
                    if (local_8 == 4)
                    {
                        ...
                        if (local_10 == null)
                            throw new OculiServiceException(0, "Virtual Disk Service not found");
                        if (local_10.Status != ServiceControllerStatus.Running)
                        {
                            logger.Verbose("Restarting Virtual disk services");
                            local_10.Start();
                        }
                    }
                }
            ...
            {
                logger.Verbose("diskpart appears to be hung, terminating diskpart process");
                logger.Verbose(local_4);
                logger.Verbose(local_5);
                return false;
            }
            ...
            {
                logger.Warning("Exception caught while partitioning the drive -- " + exception_1.ToString());
                return false;
            }
            finally
            {
                for (int local_13 = 0; local_13 < 3; ++local_13)
                {
                    ...
                    logger.Verbose(string.Format("Failed to remove our diskpart command script \"{0}\"", (object)local_2));
                }
            }
        }
    }
    ...
    public static string Encrypt(string clear)
    {
        if (!string.IsNullOrEmpty(clear))
            return Cryptor.Encrypt(clear);
        return clear;
    }

    public static string Decrypt(string clear)
    {
        if (!string.IsNullOrEmpty(clear))
            return Cryptor.Decrypt(clear);
        return clear;
    }
    ...
    public static void GetUsernameAndDomain(string username, string domain, out string usernamePart, out string domainPart)
    {
        ...
        if (string.IsNullOrEmpty(domain))
        {
            CUtils.SplitUsernameAndDomain(username, out usernamePart, out domainPart);
        }
        else
        {
